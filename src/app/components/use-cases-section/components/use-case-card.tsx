...
>(({ icon, title, description, ...props }, ref) => {
  return (
    <motion.div
      ref={ref}
      {...props}
      className="w-full max-w-[274px] rounded-2xl border border-[#262626] p-px"
    >
      <div className="relative h-full w-full rounded-[15px] border border-transparent bg-transparent p-4 md:p-7">
        ...
        <div className="relative h-12 w-12 md:h-16 md:w-16">
          ...
          <Image
            ...
            width={32}
            height={32}
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 md:w-10 md:h-10"
          />
        </div>
        <h3 className="mt-3 md:mt-6 text-lg md:text-xl font-medium text-white leading-tight">
          {title}
        </h3>
        <p className="mt-2 text-sm md:text-base text-[#6B6B6B] leading-relaxed">
          {description}
        </p>
      </div>
    </motion.div>
